/************************************************************/
/* Macro for creating basic histograms (pt, dedx vs pt,     */
/* M vs pt, eta, etc) for proton and pi+ recognition in mpd.*/
/* Macro uses list of mpddst.root files as input.           */
/*                                                          */
...
/************************************************************/
 ...
 void AnalProtonPi(TString infile = "./urqmd34-11gev.list.txt", Float_t prob = 0.5, Float_t tofMass = 0.5)
{
 TStopwatch timer;
 timer.Start();
 ...
 for (Int_t i = 0; i < events; i++){
   ...
   for (Int_t DSTtrackIndex = 0; DSTtrackIndex < fNtracks; DSTtrackIndex++){
	 ...
	 continue;
       ...
       if(primVert.Mag() > 0.50){ //only protons close to the collision
	 //cout<<"Too far"<<endl;
	 continue;
       }
