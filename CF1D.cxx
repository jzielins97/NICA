...
const Double_t kStarMin = 0.0;
const Double_t kStarMax = 0.5;
const Int_t NEVPACK = 10, NBINS = 100, kMax=1000000;
...
double getWeight(int iLL, double* mom1,double* pos1,double* mom2,double* pos2){
	//FsiInit(iLL);
...
void Cf(int nev, double Rinv, int iLL, TTree *eventTree, std::vector<L0> *l0Ev, std::vector<PP> *pEv, TFile *ofile, TH1D **num, TH1D **den, TH1D **cf, TH2D **hWeight, int wWeights){
  ...
  for(int ipt=0; ipt<NPT; ipt++){
    if(iLL == 2){
      num[ipt] = new TH1D("hCFnum_"+binnames[ipt]+"_"+systems[0],"hCFnum"+binnames[ipt],NBINS, kStarMin, kStarMax);
      den[ipt] = new TH1D("hCFden_"+binnames[ipt]+"_"+systems[0],"hCFden"+binnames[ipt],NBINS, kStarMin, kStarMax);
      cf[ipt] = new TH1D("hCF_"+binnames[ipt]+"_"+systems[0], "hCF"+binnames[ipt],NBINS, kStarMin, kStarMax);
      hWeight[ipt] = new TH2D("hWeight_"+systems[0], "weight vs k*;k*[GeV];wLed;",NBINS,0.0,0.5,NBINS, -4.0,4.0);
      pdg1 = pdgCodePr;
      pdg2 = pdgCodePr;
    }else if(iLL == 27){
      num[ipt] = new TH1D("hCFnum_"+binnames[ipt]+"_"+systems[1],"hCFnum"+binnames[ipt],NBINS, kStarMin, kStarMax);
      den[ipt] = new TH1D("hCFden_"+binnames[ipt]+"_"+systems[1],"hCFden"+binnames[ipt],NBINS, kStarMin, kStarMax);
      cf[ipt] = new TH1D("hCF_"+binnames[ipt]+"_"+systems[1], "hCF"+binnames[ipt],NBINS, kStarMin, kStarMax);
      hWeight[ipt] = new TH2D("hWeight_"+systems[1], "weight vs k*;k*[GeV];wLed;",NBINS,0.0,0.5,NBINS, -4.0,4.0);
      pdg1 = pdgCodePr;
      pdg2 = pdgCodeL0;
    }else if(iLL == 29){
      num[ipt] = new TH1D("hCFnum_"+binnames[ipt]+"_"+systems[2],"hCFnum"+binnames[ipt],NBINS, kStarMin, kStarMax);
      den[ipt] = new TH1D("hCFden_"+binnames[ipt]+"_"+systems[2],"hCFden"+binnames[ipt],NBINS, kStarMin, kStarMax);
      cf[ipt] = new TH1D("hCF_"+binnames[ipt]+"_"+systems[2], "hCF"+binnames[ipt],NBINS, kStarMin, kStarMax);
      hWeight[ipt] = new TH2D("hWeight_"+systems[2], "weight vs k*;k*[GeV];wLed;",NBINS,0.0,0.5,NBINS, -4.0,4.0);
      pdg1 = pdgCodeL0;
      pdg2 = pdgCodeL0;
    }else if(iLL == 30){
      num[ipt] = new TH1D("hCFnum_"+binnames[ipt]+"_"+systems[3],"hCFnum"+binnames[ipt],NBINS, kStarMin, kStarMax);
      den[ipt] = new TH1D("hCFden_"+binnames[ipt]+"_"+systems[3],"hCFden"+binnames[ipt],NBINS, kStarMin, kStarMax);
      cf[ipt] = new TH1D("hCF_"+binnames[ipt]+"_"+systems[3], "hCF"+binnames[ipt],NBINS, kStarMin, kStarMax);
      hWeight[ipt] = new TH2D("hWeight_"+systems[3], "weight vs k*;k*[GeV];wLed;",NBINS,0.0,0.5,NBINS, -4.0,4.0);
      pdg1 = pdgCodePr;
      pdg2 = pdgCodeApr;
    }
  }
  ...
  for(Int_t i=0; i<nev; i+=NEVPACK){ //event loop
	...
	if (fabs(mKStarSigned)>kStarMax) continue;
	...
	//calculating weights
	FsiInit(iLL);
	double tWLed = getWeight(iLL, mom1,pos1,mom2,pos2);
	if ( (isnan(tWLed))) {
	...
	for(int ipt = 0; ipt<NPT; ipt++){
	  hWeight[ipt]->Fill(TMath::Abs(mKStarSigned),tWLed);
  ...
  for(int ipt=0; ipt<NPT; ipt++){
    ...
    if(hist_den->GetEntries() == 0) continue;
...
//opening file with protons and lambdas
	TFile* ifile = new TFile(Form("./data/combined_%.1f.root",energy));///data/100kev/pL_0-100k.hist.root");//xi-1.histo.root");
...
//opening output file for all histograms
	TFile* ofile;
	if(wWeights) ofile = new TFile(Form("CF_%.1fGeV_%d_R%.1f_Weights.root",energy,nev,Rinv), "recreate");
	else ofile = new TFile(Form("CF_%.1fGeV_%d_R%.1f_noWeights.root",energy,nev,Rinv), "recreate");
